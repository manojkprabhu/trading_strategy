...
#BarUp BULL CALL
data['entry_signal_bull'] = (data['short_ema'].shift() < data['long_ema'].shift()) & (data['short_ema'] > data['long_ema']).astype(int)
...
print("total_profit_bull: " + str(round(total_profit_bull,2)))
...
#BarDn BEAR CALL
data['entry_signal_bear'] = (data['short_ema'].shift() > data['long_ema'].shift()) & (data['short_ema'] < data['long_ema']).astype(int)
...
for i, row in data.iterrows():
    if row['exit_signal_bear'] == 1:
        if position != 0 and entry_price != None:
            ...
            total_profit_bear += round(profit,2)
            position = 0  # No position
            position_qty = 0
...
print("total_profit_bear: " + str(round(total_profit_bear,2)))
...
fig.update_layout(title='EMA Candlestick Plot with Entry and Exit Signals',
                  title_x=0.5, 
                  xaxis_title='Time',
                  yaxis_title='Price',
...
offline.plot(fig, filename='ema_candlestick_plot.html')
